...
router.get('/api/users/profile', authorize('customer', 'restaurant_owner', 'admin'), userController.getProfile);
router.put('/api/users/profile', authorize('customer', 'restaurant_owner', 'admin'), userController.updateProfile);
...
router.put('/api/users/:id/role', [
  authorize('admin'),
  body('role', 'Valid role is required (customer, restaurant_owner, admin)')
